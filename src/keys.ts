 ...
 * Set DEMO_SILMARILS_MASTER_SEED only to match the Rust testnet CLI fixed seed.
 */
export function loadSignerConfig(): SignerConfig {
  const skEnv = process.env.DEMO_SPHINCS_SK_HEX?.trim()
  if (!skEnv) {
    throw new Error(
      'DEMO_SPHINCS_SK_HEX is required (64-byte SPHINCS+ secret, hex). ' +
        'Use the same demo private keys as the web wallet — see README.',
    )
  }
  ...
  const pkEnv = process.env.DEMO_SPHINCS_PK_HEX?.trim()
...
function resolveSilmarilsMasterKey(sphincsSecretKey: Uint8Array): Uint8Array {
  const rustCliSeed = process.env.DEMO_SILMARILS_MASTER_SEED?.trim()
  if (rustCliSeed) {
